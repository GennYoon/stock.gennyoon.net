  ...
  try {
    ...
    if (weeklyGroup) {
      ...
      ['A', 'B', 'C', 'D'].forEach(groupName => {
        const existingGroup = groupsArray.find(g => g.group_name === groupName);
        if (existingGroup) {
    ...
    const groupsWithDividends = await Promise.all(
      groupsArray.map(async (group) => {
        try {
          ...
          if (stocksError) {
            console.error(`Error fetching stocks for ${group.issuer} ${group.group_name}:`, stocksError);
            return group;
          }
            ...
            try {
              ...
              if (response.ok) {
                ...
                if (data.results && data.results.length > 0) {
                  ...
                  const exDate = new Date(dividend.ex_dividend_date + 'T09:00:00+09:00');
                  const payDate = new Date(dividend.pay_date + 'T09:00:00+09:00');
              ...
              console.error(`Error fetching dividend for ${stock.ticker}:`, error);
            }
          }
          ...
          console.error(`Error fetching dividend for ${group.issuer} ${group.group_name}:`, error);
        }
    ...
    const sortedGroups = groupsWithDividends
      ...
      .sort((a, b) => (a.time_until_ex_date || 0) - (b.time_until_ex_date || 0));
